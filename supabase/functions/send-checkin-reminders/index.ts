...
//
// Non tocca MAI il Vault: legge solo trips.prefs->>'departureAt', un
// campo inserito a mano apposta per questo (vedi tab Viaggio), separato
// dai documenti del Vault che restano locali sul dispositivo.
//
...
Deno.serve(async (req: Request) => {
  ...
  for (const trip of (trips || []) as TripRow[]) {
    const departureAt = trip.prefs && trip.prefs.departureAt;
    ...
    const { data: subs, error: subsErr } = await supabase
      ...
      .select("id, endpoint, p256dh, auth, notified_for")
      .eq("trip_id", trip.id);
    if (subsErr || !subs) continue;

    for (const sub of subs as SubRow[]) {
      if (sub.notified_for === departureAt) { skipped++; continue; }

      const payload = JSON.stringify({
        title: "✈️ Check-in volo",
        body: `Il check-in per il tuo volo potrebbe essere aperto: partenza tra circa ${HOURS_BEFORE} ore.`,
        tag: "checkin-" + trip.id,
        url: "./",
      });
      ...
      try {
        ...
        sent++;
        await supabase.from("push_subscriptions").update({ notified_for: departureAt }).eq("id", sub.id);
      } catch (e) {
